<div align="center">

<img src="./assets/main-banner.png" width="100%" alt="Kyoung9's GitHub" />

<br/>

[![Hackathon Winner Banner](./assets/hack-1.png)](https://student.redesigner.jp/portfolios/PF82b29f7d8c738912ce7e5edaad1b6103)

<br/>

![GitHub Streak](https://streak-stats.demolab.com?user=Kyoung9&theme=tokyonight&hide_border=true&background=00000000)

</div>

---

## 🐾 About

```yaml
kyoung9:
  name: Kyoungpin Gwak
  currently: "building small tools from tiny ideas"
  interests:
    - web apps
    - AI-powered tools
    - automation
    - security research
  research: entropy-based ransomware detection
  favorite_stack:
    - Next.js
    - TypeScript
    - Python
    - AWS
  languages:
    - Korean
    - Japanese
    - English
  companion: cat
```

---

## 🧰 Skills Inventory

<p align="left">
  <img src="https://img.shields.io/badge/Next.js-000000?style=flat-square&logo=nextdotjs&logoColor=white" />
  <img src="https://img.shields.io/badge/React-61DAFB?style=flat-square&logo=react&logoColor=black" />
  <img src="https://img.shields.io/badge/TypeScript-3178C6?style=flat-square&logo=typescript&logoColor=white" />
  <img src="https://img.shields.io/badge/Python-3776AB?style=flat-square&logo=python&logoColor=white" />
  <img src="https://img.shields.io/badge/AWS-232F3E?style=flat-square&logo=amazonaws&logoColor=white" />
  <img src="https://img.shields.io/badge/OpenAI-412991?style=flat-square&logo=openai&logoColor=white" />
  <img src="https://img.shields.io/badge/Supabase-3FCF8E?style=flat-square&logo=supabase&logoColor=white" />
  <img src="https://img.shields.io/badge/PostgreSQL-4169E1?style=flat-square&logo=postgresql&logoColor=white" />
</p>

---

## 📦 Featured Projects

| Project | Description | Stack |
|---|---|---|
| 🥃 **WhiskyFinder-JP** | Japanese whisky price search and CSV export tool | Next.js |
| 💌 **Gmail Auto-Responder** | AI-powered email automation with Gmail API and AWS Lambda | Python / AWS / OpenAI |
| 🐿️ **Senior Monitoring System** | IoT monitoring system using ESP32 and AWS | Next.js / AWS |
| 🤖 **AI Meeting Support System** | Meeting summary, task extraction, and research automation | Next.js / AI |

---

## 🔬 Research

I’m researching **entropy-based ransomware detection** and evasion techniques.  
My current interest is how detection systems can remain reliable when attackers try to bypass entropy-based features.

---

## 📊 GitHub Stats

<div align="center">

![Top Langs](https://github-readme-stats.shion.dev/api/top-langs/?username=Kyoung9&layout=compact&theme=tokyonight&hide_border=true&bg_color=00000000)

</div>

---

## 🔗 Connect Portal

<p align="left">
  <a href="mailto:gwakkyoungpin@gmail.com">
    <img src="https://img.shields.io/badge/Email-111111?style=for-the-badge&logo=gmail&logoColor=EA4335" />
  </a>
  <a href="https://www.linkedin.com/in/kyoungpin-gwak-b953153a8">
    <img src="https://img.shields.io/badge/LinkedIn-111111?style=for-the-badge&logo=linkedin&logoColor=0A66C2" />
  </a>
</p>

<p align="left">
  <img src="https://komarev.com/ghpvc/?username=Kyoung9&label=PROFILE+VIEWS&color=1abc9c&style=flat-square" />
</p>